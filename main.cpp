...
void ExportJsonReport(const std::string& jsonPath, const std::vector<TargetReport>& reports) {
    try {
        ...
        jsonFile << "{\n";
        jsonFile << "  \"engine\": \"system-cleaner-agent v5.7.0 (C++17 ReAct Agentic Engine)\",\n";
        jsonFile << "  \"targets\": [\n";
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "version" || cmd == "--version" || cmd == "-v") {
        std::cout << "system-cleaner-agent v5.7.0 (C++17 Autonomous ReAct Agentic Engine - 64-bit Architecture)\n";
        return 0;
    }
